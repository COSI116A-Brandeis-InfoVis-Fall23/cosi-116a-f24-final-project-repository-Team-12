...
function linechart() {
    ...
    function createlinechart(selector, data, stopn){
        olddata=data;
        data = data.filter(d => d['stop_name'] === stopn && d['dir_id'] === 'NB');
        ...
        svg = d3.select(selector)
          ...
          let yAxis = svg.append("g")
            ...
            const inflowData = data.map(d => ({ timeperiod: d.time_period_name, value: d.total_ons }));
            const outflowData = data.map(d => ({ timeperiod: d.time_period_name, value: d.total_offs }));
